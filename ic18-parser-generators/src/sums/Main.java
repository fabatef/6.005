...
public class Main {
    ...
    public static Sum parse(String string) {
       ...
       // *** debugging option #1: print the tree to the console // comment out on ps2
       System.err.println(tree.toStringTree(parser));

       // *** debugging option #2: show the tree in a window  // comment out on ps2
       Trees.inspect(tree, parser);
       
       // *** debugging option #3: walk the tree with a listener  // comment it out on ps2
...
class MakeSum implements SumListener {
    ...
    @Override public void exitProduct (SumParser.SumContext context) {  
        // matched addend ('+' addend)+
        List<SumParser.AddendContext> addends = context.addend();
        assert stack.size() >= addends.size();
