...
public class MinesweeperServer {
    ...
    //   -The board is threadsafe
	//   -Each client gets a new thread to run the board on.
     ...
     * @param the number of users including this client using the server
     * @throws IOException if the connection encounters an error or terminates unexpectedly
     */
    private void handleConnection(Socket socket, Integer users) throws IOException {
        BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
        PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
        out.println("Welcome to Minesweeper. Board: "+ String.valueOf(board.getDimension().get(0))+
        		" columns by " +  String.valueOf(board.getDimension().get(1))+ " rows. Players: " +
        		users + " including you. Type 'help' for help.");
        ...
        try {
            ...
            for (line = in.readLine(); line != null; line = in.readLine()) {
                ...
                if (output != null) {
                	out.print(output);
                	out.flush();
                	if ((output.equals(board.boom()) && !debug)|| output.equals("bye" + System.getProperty("line.separator"))){
                    	socket.close();
                    	break;
                    }              
                }
    ...
    private String handleRequest(String input) {
        ...
        if (tokens[0].equals("look")) {
            ...
            // 'bye' request
        	return "bye" + System.getProperty("line.separator");
        } else {
    ...
    public static void runMinesweeperServer(boolean debug, Optional<File> file, int sizeX, int sizeY, int port) throws IOException {
        Board board;
        if (file.isPresent()){
        	File userfile= file.get();
        	board= new Board(userfile);
        }else{
